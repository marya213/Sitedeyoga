      ...
      >
        <div className="section-inner">
          <div className="text-center mb-8">
            <h2 className="font-serif font-light text-3xl">Cours réguliers</h2>
          </div>
          ...
          >
            {COURS_FILTERS.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setCoursFilter(tab.key)}
                className="btn"
                style={{
                  padding: ".4rem 1rem",
                  fontSize: ".8125rem",
                  fontWeight: coursFilter === tab.key ? 600 : 400,
                  background:
                    coursFilter === tab.key ? "#2D1B4E" : "transparent",
                  color:
                    coursFilter === tab.key
                      ? "#C9A84C"
                      : "var(--color-ink-muted)",
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {filteredCours.map((c) => (
              <CourseCard
                ...
                course={c}
                ctaLabel={
                  c.mode === "presentiel" ? "Réserver" : "Essai gratuit"
                }
                ctaTo="/inscription"
              />
            ))}
